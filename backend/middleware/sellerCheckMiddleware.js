  ...
  if (req.params.itemId) {
    try {
      ...
      if (item.sellerId !== req.user._id) {
        return res.status(403).json({ message: "You do not have permission to edit or delete this item" });
      }

      // If the seller check passes, proceed to the next middleware or route handler
      next();
    } catch (error) {
      return res.status(500).json({ message: "Error verifying item seller", error: error.message });
    }
  } else {
